...
export const campusMembersTable = pgTable("campus_members", {
  ...
  expiresOn: date("expires_on", { mode: "string" }).notNull().default("2027-06-30"),
...
export const insertCampusMemberSchema = createInsertSchema(campusMembersTable).omit({
  createdAt: true,
  updatedAt: true,
});
export const insertCampusSpaceSchema = createInsertSchema(campusSpacesTable).omit({
  createdAt: true,
});
export const insertCampusMembershipSchema = createInsertSchema(campusMembershipsTable).omit({
  createdAt: true,
});
...
export const campusAttendanceTable = pgTable("campus_attendance", {
  id: text("id").primaryKey(),
  spaceId: text("space_id").notNull(),
  studentId: text("student_id").notNull(),
  ...
  attendanceUnique: uniqueIndex("campus_attendance_unique").on(table.spaceId, table.studentId, table.date),
}));

export const insertCampusAssignmentSchema = createInsertSchema(campusAssignmentsTable).omit({ createdAt: true });
export const insertCampusSubmissionSchema = createInsertSchema(campusSubmissionsTable).omit({ submittedAt: true });
export const insertCampusAttendanceSchema = createInsertSchema(campusAttendanceTable).omit({ recordedAt: true });
...
export const campusChannelsTable = pgTable("campus_channels", {
  id: text("id").primaryKey(),
  spaceId: text("space_id").notNull(),
  name: text("name").notNull(),
...
export const campusMessagesTable = pgTable("campus_messages", {
  id: text("id").primaryKey(),
  ...
  channelCreatedIdx: index("campus_messages_channel_created_at_idx").on(table.channelId, table.createdAt),
}));
...
export const campusProjectMembersTable = pgTable("campus_project_members", {
  id: text("id").primaryKey(),
  ...
  projectMemberUnique: uniqueIndex("campus_project_members_unique").on(table.projectId, table.userId),
  projectIdx: index("campus_project_members_project_id_idx").on(table.projectId),
  userIdx: index("campus_project_members_user_id_idx").on(table.userId),
}));

export const campusProjectMilestonesTable = pgTable("campus_project_milestones", {
  id: text("id").primaryKey(),
  projectId: text("project_id").notNull(),
  title: text("title").notNull(),
  ...
  projectIdx: index("campus_project_milestones_project_id_idx").on(table.projectId, table.createdAt),
}));
...
export type CampusProjectMilestone = typeof campusProjectMilestonesTable.$inferSelect;
...
export const campusMemberSkillsTable = pgTable("campus_member_skills", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull(),
  skillId: text("skill_id").notNull(),
  proficiency: text("proficiency").notNull().default('intermediate'),
}, (table) => ({
  memberSkillUnique: uniqueIndex("campus_member_skills_unique").on(table.userId, table.skillId),
}));
...
export const campusEventAttendeesTable = pgTable("campus_event_attendees", {
  id: text("id").primaryKey(),
  ...
  eventAttendeeUnique: uniqueIndex("campus_event_attendees_unique").on(table.eventId, table.userId),
}));

export const campusClubApplicationsTable = pgTable("campus_club_applications", {
  id: text("id").primaryKey(),
  spaceId: text("space_id").notNull(),
  userId: text("user_id").notNull(),
  ...
  clubApplicationUnique: uniqueIndex("campus_club_applications_unique").on(table.spaceId, table.userId),
}));
...
export type CampusClubApplication = typeof campusClubApplicationsTable.$inferSelect;
...
export const campusVerificationAuditsTable = pgTable("campus_verification_audits", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull(),
...
export const campusNotificationsTable = pgTable("campus_notifications", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull(),
  kind: text("kind").notNull(),
  title: text("title").notNull(),
...
export const campusPostReactionsTable = pgTable("campus_post_reactions", {
  id: text("id").primaryKey(),
  postId: text("post_id").notNull(),
  userId: text("user_id").notNull(),
  ...
  uniqueReaction: uniqueIndex("campus_post_reactions_unique").on(table.postId, table.userId, table.reaction),
}));
...
export const campusModerationReportsTable = pgTable("campus_moderation_reports", {
  id: text("id").primaryKey(),
  reporterId: text("reporter_id").notNull(),
  ...
  targetIdx: index("campus_moderation_reports_target_idx").on(table.targetType, table.targetId),
  statusIdx: index("campus_moderation_reports_status_idx").on(table.status, table.createdAt),
  reporterIdx: index("campus_moderation_reports_reporter_idx").on(table.reporterId, table.createdAt),
}));

export type CampusVerificationAudit = typeof campusVerificationAuditsTable.$inferSelect;
export type CampusModerationReport = typeof campusModerationReportsTable.$inferSelect;
