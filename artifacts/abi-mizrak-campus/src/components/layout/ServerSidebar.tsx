...
const accent = (name?: string) => {
  const map: Record<string, string> = {
    emerald: "bg-[hsl(var(--lc-accent)/.12)] text-[var(--lc-accent)]",
    ...
    <nav className="flex w-[72px] shrink-0 flex-col items-center gap-3 overflow-y-auto border-r border-[var(--lc-line)] bg-white/10 backdrop-blur-xl py-5 hide-scrollbar">
      ...
      <Link href="/">
        <div className="group relative flex w-full justify-center">
          <div
            className={`absolute left-0 top-1/2 h-8 w-1 -translate-y-1/2 rounded-r-full bg-[#25423A] transition-all ${isHome ? "opacity-100 scale-100" : "opacity-0 scale-0 group-hover:opacity-100 group-hover:scale-75"}`}
          />
          <button
            className={`flex h-12 w-12 items-center justify-center rounded-2xl liquid-surface deep font-bold text-white shadow-[0_16px_40px_hsl(var(--lc-shadow)/.18)] transition-all ${isHome ? "rounded-[16px]" : "hover:rounded-[16px]"}`}
          >
            AM
          </button>
        </div>
        ...
        joinedSpaces.map((space) => {
          ...
          return (
            <Link key={space.id} href={`/spaces/${space.id}`}>
              ...
              >
                <div
                  className={`absolute left-0 top-1/2 h-8 w-1 -translate-y-1/2 rounded-r-full bg-[#25423A] transition-all ${isActive ? "opacity-100 scale-100" : "opacity-0 scale-0 group-hover:opacity-100 group-hover:scale-75"}`}
                />
                <button
                  className={`flex h-12 w-12 items-center justify-center rounded-full transition-all ${isActive ? "rounded-[16px]" : "hover:rounded-[16px]"} ${colorClass}`}
                >
                  <Icon size={20} />
                </button>
              </div>
