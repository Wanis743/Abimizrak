...
import { Home, Compass, Activity, Calendar, FileBadge2, UserCheck, Hash, LogOut, ChevronDown, Network } from 'lucide-react';
...
export function ChannelSidebar({ role, spaceId }: ChannelSidebarProps) {
  ...
  const homeNavItems = [
    ...
    ...(role === 'teacher' || role === 'admin' ? [{ href: '/teacher', label: 'Teacher workspace', icon: UserCheck }] : []),
    ...(role === 'admin' ? [{ href: '/admin', label: 'Admin center', icon: UserCheck }, { href: '/admin/verification', label: 'Verify members', icon: UserCheck }] : []),
  ];

  // Space Context Navigation (Placeholder channels for now)
  const spaceNavItems = spaceId ? [
    { href: `/spaces/${spaceId}`, label: 'feed', icon: Hash },
    { href: `/spaces/${spaceId}/assignments`, label: 'assignments', icon: Hash },
    { href: `/spaces/${spaceId}/resources`, label: 'resources', icon: Hash },
    ...(role === 'teacher' || role === 'admin' ? [{ href: `/spaces/${spaceId}/teacher`, label: 'Teacher Workspace', icon: UserCheck }] : []),
  ] : [];
  ...
  return (
    <aside className="flex w-[240px] shrink-0 flex-col border-r border-[var(--lc-line)] bg-white/8 backdrop-blur-2xl">
      ...
      <div className="flex h-[60px] shrink-0 items-center justify-between border-b border-[var(--lc-line)] px-4 font-bold text-[var(--lc-ink)]">
        <h2 className="truncate text-base">{isHome ? 'Lycée Abi Mizrak' : space?.name || 'Loading...'}</h2>
            ...
            {homeNavItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}>
                  <div className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm font-bold transition-colors ${isActive ? 'bg-[hsl(var(--lc-accent)/.12)] text-[var(--lc-accent)]' : 'text-[var(--lc-muted)] hover:bg-white/10 hover:text-[var(--lc-ink)]'}`}>
                    <item.icon size={18} />
                    {item.label}
                  </div>
                </Link>
          ...
          <div className="space-y-0.5">
            {spaceNavItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}>
                  <div className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm font-bold transition-colors ${isActive ? 'bg-[hsl(var(--lc-accent)/.12)] text-[var(--lc-accent)]' : 'text-[var(--lc-muted)] hover:bg-white/10 hover:text-[var(--lc-ink)]'}`}>
                    <item.icon size={18} className={item.label === 'Teacher Workspace' ? 'text-[#8F3D2E]' : 'text-[var(--lc-muted)]'} />
                    {item.label}
                  </div>
                </Link>
              );
      ...
      <div className="mt-auto flex shrink-0 items-center justify-between border-t border-[var(--lc-line)] bg-white/10 p-3">
        <div className="flex items-center gap-2 overflow-hidden">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#25423A] text-xs font-bold text-white">
            {user?.fullName?.split(' ').map((part: string) => part[0]).join('').slice(0, 2).toUpperCase() || 'U'}
          </div>
          <div className="truncate">
            <div className="truncate text-xs font-bold text-[var(--lc-ink)]">{user?.fullName || 'User'}</div>
            <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--lc-muted)]">{role}</div>
          </div>
        </div>
        <button onClick={() => signOut()} className="rounded-md p-1.5 text-[#59706A] hover:bg-[#DDE8DF] hover:text-[var(--lc-ink)]" title="Sign Out">
          <LogOut size={16} />
        </button>
      </div>
    </aside>
